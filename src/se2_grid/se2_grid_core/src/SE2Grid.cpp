...
namespace se2_grid
{
    ...
    bool SE2Grid::short_astar(const Eigen::Vector2d &start_pt, 
                          const Eigen::Vector2d &end_pt, 
                          std::vector<Eigen::Vector2d>& path,
                          std::string layer)
    {
      path.clear();
      GridNodePtr* grid_node_map = new GridNodePtr[size_pos(0) * size_pos(1)];
      for(int i = 0 ; i < size_pos(0) * size_pos(1); i++)
          grid_node_map[i] = new GridNode();

      if (!isInMap(Eigen::Vector3d(start_pt(0), start_pt(1), 0.0))|| 
          !isInMap(Eigen::Vector3d(end_pt(0), end_pt(1), 0.0)))
        return false;

      std::multimap<double, GridNodePtr> openSet;
      Eigen::Vector2i start_index = posToIndexAstar(start_pt);
      GridNodePtr start_point = grid_node_map[toAddressAstar(start_index)];
      start_point->index = start_index;
      GridNodePtr currentPtr  = NULL;
      GridNodePtr neighborPtr  = NULL;

      openSet.insert(std::make_pair(0.0, start_point));
      Eigen::Vector2i end_index = posToIndexAstar(end_pt);
      ...
      while ( !openSet.empty() )
      {
        ...
        openSet.erase(iter);
        grid_node_map[toAddressAstar(currentPtr->index)]->id = -1;
        ...
        if( currentPtr -> index == end_index )
        {
            ...
            while (p -> father != NULL)
            {
                path.push_back(indexToPosAstar(p->index));
                p = p -> father;
            }
            path.push_back(indexToPosAstar(p->index));
            for (int i = 0 ; i < size_pos(0) * size_pos(1); i++)
        ...
        for(int i = -1; i <= 1; i++)
        {
          for(int j = -1; j <= 1; j++)
          {
            ...
            Eigen::Vector2d neighbor_pos_2d = indexToPosAstar(neighbor_index);
            Eigen::Vector3d neighbor_pos(neighbor_pos_2d(0), neighbor_pos_2d(1), 0.0);
            if(isInMap(neighbor_pos))
            {  
              neighborPtr = grid_node_map[toAddressAstar(neighbor_index)];
              bool occ = value(layer, neighbor_pos) > 0.5? true : false;
              if (neighborPtr->id == 0 && !occ)
              {
                  double tg = ((i * j == 0) ? resolution_pos : (resolution_pos * 1.41)) + currentPtr -> gScore;
                  double heu = (end_index - neighbor_index).norm() * resolution_pos;
