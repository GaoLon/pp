...
namespace se2_grid 
{
  ...
  */
  ...
  public:
    ...
    bool short_astar(const Eigen::Vector2d &start_pt, 
    ...
    inline void boundPos(Eigen::Vector3d& pos) const
    {
      ...
      positionShifted += position - origin;
      pos(0) = positionShifted(0); 
      pos(1) = positionShifted(1);
      return;
    }
  ...
  private:
    ...
    inline bool getLinearInterpolated(const std::string& layer, const Eigen::Vector3d& pos_orin, float& value) const
    {
      Eigen::Vector3d point;
      Eigen::Array3i point_idx;

      Eigen::Vector3d pos = pos_orin;
      boundPos(pos);
      ...
      else
      {
        ...
        Eigen::Vector2d positionRed;
        positionRed(0) = (pos(0) - point(0)) * resolution_pos_inv;
        positionRed(1) = (pos(1) - point(1)) * resolution_pos_inv;
        const Eigen::Vector2d positionRedFlip = Eigen::Vector2d::Ones() - positionRed;
    ...
    inline bool getLinearInterpolatedGrad(const std::string& layer, const Eigen::Vector3d& pos_orin,
    ...
    inline Eigen::Vector2i posToIndexAstar(Eigen::Vector2d pos)
    {
      Eigen::Vector2i index;
      Eigen::Vector2d idx_pos = (origin + position - pos) / resolution_pos;
      index(0) = std::min(std::max((int)std::floor(idx_pos(0)), 0), size_pos(0) - 1);
      index(1) = std::min(std::max((int)std::floor(idx_pos(1)), 0), size_pos(1) - 1);
      return index;
    } 

    inline Eigen::Vector2d indexToPosAstar(Eigen::Vector2i index)
    {
      Eigen::Vector2d posw;
      posw = (origin + position) - index.cast<double>() * resolution_pos;
      posw(0) -= 0.5 * resolution_pos;
      posw(1) -= 0.5 * resolution_pos;
      return posw;
    }

    inline int toAddressAstar(Eigen::Vector2i index)
    {
      int idx = index(0) * size_pos(1) + index(1);
      int size_all = size_pos(0) * size_pos(1);
      return std::max(std::min(idx, size_all - 1), 0);
    }
  ...
  private:
    ...
    std::string frame_id = std::string("world");
